...
of solving almost any text processing problem~». C'est un puissant outil de
text mining, mais pas facile à installer et configurer si on souhaite un
serveur de catégorisation avec une API.

\subsubsection{Textimate}

Textimate\footnote{\url{http://textimate.me/}} est un service web qui propose
une API permettant d'entraîner et de requêter des classifiers pour catégoriser
des textes et faire de la détection de spams. Le code source est propriétaire
(non accessible) et le service est payant à partir de 500 requêtes par mois.

\subsubsection{uClassify}

uClassify\footnote{\url{http://www.uclassify.com/}} est un service web gratuit
qui propose une API permettant d'entraîner et de requêter des classifiers pour
catégoriser des textes, faire de la détection de spams, ou détecter le gender,
l'âge, ou encore le mood de l'auteur d'un texte.

Bien que l'utilisation de l'API soit gratuite, le code source est propriétaire,
il fonctionne sur la plateforme Windows. uClassify propose des licences
payantes pour l'installation d'une instance serveur privée.
